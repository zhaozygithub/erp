...
public class MyCustomerController extends BaseController {
	...
	public void index() {
		...
		tableHeader.setFieldNames(new String[] { "id", "name", "phone","cn_type",
				"belong_user_id", "belong_user_name", "belong_org_id",
				"belong_org_name", "cn_status" , "cn_cu_status_remark"});
		tableHeader.setCNNames(new String[] { "客户编号", "名称", "电话号码", "客户类型","管户人编号",
		...
		try {
			...
			// excel文件的文件名称
			response = PageUtil.createTablePageStructure("/myCustomer/data",
					"我的客户列表数据", tableHeader, search, super.getLastPara().toString(), (Map<Integer, SysMenu>) this.getSessionAttr("menus"));
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void data() {
		...
		whereList.add(new QueryWhere("belong_user_id",user.getInt("id")));
	...
	public void toAdd() {
		...
		cuObjectCustomer.set("belong_user_id",user.getInt("id"));
